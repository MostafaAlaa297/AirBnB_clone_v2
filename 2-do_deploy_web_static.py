...
def do_deploy(archive_path):
    """Distributes an archive to web servers"""
        ...
        try:
            ...
            run('tar -xzf {} -C {}'.format(remote_tmp_path, remote_extract_path))

            # Remove the archive
            run('rm {}'.format(remote_tmp_path))

            # Move contents of extracted folder to release folder
            run('mv {}/web_static/* {}'.format(remote_extract_path, remote_extract_path))
            ...
            # Create new symlink
            run('ln -s {} /data/web_static/current'.format(remote_extract_path))
